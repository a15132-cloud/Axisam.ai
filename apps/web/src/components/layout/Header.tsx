...
export function Header({
  ...
  return (
    <header className="flex flex-col gap-3 border-b border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-3 sm:px-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <div className="min-w-0">
            <h1 className="truncate text-sm font-semibold text-[var(--color-text)]">{nombreProyecto}</h1>
          ...
          {(["chat", "detalles"] as const).map((v) => (
            <button
              ...
              className={`rounded-md py-2 text-sm font-medium capitalize transition-colors ${
                vistaMobile === v ? "bg-[var(--color-accent)] text-white" : "text-[var(--color-text-muted)]"
              }`}
            >
