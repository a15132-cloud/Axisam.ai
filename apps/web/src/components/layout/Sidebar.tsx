...
export function Sidebar({ proyectos, proyectoActualId, onSeleccionar, onNuevoProyecto, onEliminar, creando }: SidebarProps) {
  return (
    <aside className="flex h-full w-64 shrink-0 flex-col border-r border-[var(--color-border)] bg-[var(--color-surface)]">
      ...
      <div className="px-3">
        <motion.button
          ...
          disabled={creando}
          className="flex w-full items-center justify-center gap-2 rounded-lg bg-[var(--color-accent)] px-3 py-2.5 text-sm font-medium text-white transition-colors hover:bg-[var(--color-accent-2)] disabled:opacity-60"
        >
        ...
        {proyectos.map((p) => (
          <div key={p.id} className="group relative">
            ...
            <button
              ...
              className="absolute right-2 top-1/2 hidden -translate-y-1/2 rounded-md p-1 text-[var(--color-text-faint)] hover:text-[var(--color-danger)] group-hover:block"
